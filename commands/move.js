        ...
        if(lobby)
        {
            if(interaction.user.id == lobby.player1.id || interaction.user.id == lobby.player2.id)
            {
                if(interaction.user.id == lobby.player1.id && lobby.player1.turn || interaction.user.id == lobby.player2.id && lobby.player2.turn)
                {
                    ...
                    {
                        const fromX = values.find(obj => from[0] == obj.name).value
                        const fromY = values.find(obj => from[1] == obj.name).value
                        const toX = values.find(obj => to[0] == obj.name).value
                        const toY = values.find(obj => to[1] == obj.name).value
                        ...
                        if(lobby.board[fromX][fromY].name)
                        {
                            ...
                            {
                                const attachment = await Board.CreateBoard(from, fromX, fromY, to, toX, toY, lobby)
